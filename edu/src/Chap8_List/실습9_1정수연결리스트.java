...
class Node1 {
	...
	public Node1(int element) {
		data = element;//노드에 저장되는 데이터
		link = null;//다음 노드를 가르키는 링크(포인터)
	}
}

//LinkedList에 대해 여러 가지 연산을 수행하는 코드. 
//메서드들: Delete(int element), Show(), Add(int element), Search(int data), Merge(LinkedList1 b).
class LinkedList1 {//LinkedList의 기본 기능을 구현하는 클래스
	Node1 first;//연결 리스트의 첫 번째 노드를 가르키는 포인터

	public LinkedList1() {//LinkedList1 클래스의 생성자
		first = null;//생성자에서 first(머리포인터)를 null로 초기화하여 빈 리스트로 시작.
	}
...
public class 실습9_1정수연결리스트 {
	enum Menu {//프로그램에서 사용할 메뉴 항목을 정의한 열거형.//메서드: MenuAt(int idx), getMessage().
		Add("삽입"), Delete("삭제"), Show("인쇄"), Search("검색"), Merge("합병"), Exit("종료");

		private final String message; // 표시할 문자열

		static Menu MenuAt(int idx) { //주어진 인덱스에 해당하는 메뉴 항목을 반환// 순서가 idx번째인 열거를 반환
		...
		String getMessage() { // 각 메뉴 항목에 해당하는 메시지를 반환
			return message;
		}
	} 

	// --- 메뉴 선택 ---//
	static Menu SelectMenu() {//사용자가 메뉴 항목을 선택할 수 있도록 하는 메서드//사용자로부터 메뉴 번호를 입력받고, 해당하는 메뉴 항목을 반환.
		...
		do {
			...
			System.out.print(" : ");
			key = sc1.nextInt();//메뉴 선택 번호로 입력된 값이 key이다 
		} while (key < Menu.Add.ordinal() || key > Menu.Exit.ordinal());//입력된 key가 음수이거나 Exit에 대한 enum 숫자보다 크면 다시 입력받는다 
		return Menu.MenuAt(key);
	}

	public static void main(String[] args) {//프로그램 진입점. 사용자가 메뉴를 선택하고, 선택한 메뉴에 맞게 동작을 수행
